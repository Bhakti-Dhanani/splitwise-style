...
function AuthFormInner({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      router.push(`/sign-in${callbackUrl ? `?callbackUrl=${encodeURIComponent(callbackUrl)}` : ''}`)
    } else {
...
export function AuthForm(props: { mode: 'sign-in' | 'sign-up' }) {
  return (
    <Suspense fallback={<div className="min-h-svh flex items-center justify-center text-muted-foreground">Loading...</div>}>
      <AuthFormInner {...props} />
    </Suspense>
  )
}
